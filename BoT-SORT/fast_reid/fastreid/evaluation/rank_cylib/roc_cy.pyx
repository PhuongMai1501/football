...
cpdef evaluate_roc_cy(float[:,:] distmat, long[:] q_pids, long[:]g_pids,
    ...
    cdef:
        ...
        float[:] pos = np.zeros(num_q*num_g, dtype=float32)
        float[:] neg = np.zeros(num_q*num_g, dtype=float32)
    ...
    cdef float[:] labels = np.hstack((np.zeros(valid_pos, dtype=float32),
                                      np.ones(valid_neg, dtype=float32)))
