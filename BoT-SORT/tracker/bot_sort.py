...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, cls=None, feat=None, feat_history=50):
        ...
        # store class id (from detector). None if unknown
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        # propagate class
        self.cls = getattr(new_track, "cls", self.cls)
...
class BoTSORT(object):
    ...
    def update(self, output_results, img):
        ...
        if len(dets) > 0:
            # ensure classes_keep is integer array
            try:
                classes_keep = np.asarray(classes_keep, dtype=int)
            except Exception:
                classes_keep = np.array([int(c) for c in classes_keep], dtype=int)

            if self.args.with_reid:
                # detections with appearance features and class
                detections = [
                    STrack(STrack.tlbr_to_tlwh(tlbr), s, int(cls_id), f)
                    for (tlbr, s, cls_id, f) in zip(dets, scores_keep, classes_keep, features_keep)
                ]
            else:
                # detections with class but no appearance feature
                detections = [
                    STrack(STrack.tlbr_to_tlwh(tlbr), s, int(cls_id))
                    for (tlbr, s, cls_id) in zip(dets, scores_keep, classes_keep)
                ]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            classes_second = np.asarray(classes_second, dtype=int) if len(classes_second) else np.array([], dtype=int)
            detections_second = [
                STrack(STrack.tlbr_to_tlwh(tlbr), s, int(cls_id))
                for (tlbr, s, cls_id) in zip(dets_second, scores_second, classes_second)
            ]
        else:
            detections_second = []
